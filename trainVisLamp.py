...
# Optimizer
opVisLamp = optim.Adam(visLampNet.parameters(), lr=1e-4, betas=(0.9, 0.999 ) )
if opt.isFineTune:
    opVisLamp.load_state_dict(visLampDict['optim'] )
...
for epoch in list(range(epochId, opt.nepoch ) ):
    ...
    for i, dataBatch in enumerate(brdfLoader ):
        ...
        if onVisLampNum > 0:
            ...
            visLampErr = geoW * visLampPointsErr + shgW * visLampShgErr + renW * visLampRenErr
            visLampErr /=  onVisLampNum
            visLampErr.backward()
            opVisLamp.step()


        # Write errors to screen
        utils.writeErrToScreen('visLampPoints', [visLampPointsErr / max(onVisLampNum, 1) ], epoch, j )
        utils.writeErrToScreen('visLampShading', [visLampShgErr / max(onVisLampNum, 1) ], epoch, j )
        utils.writeErrToScreen('visLampRender', [visLampRenErr / max(onVisLampNum, 1) ], epoch, j )

        # Write errors to file
        utils.writeErrToFile('visLampPoints', [visLampPointsErr / max(onVisLampNum, 1) ], trainingLog, epoch, j )
        utils.writeErrToFile('visLampShading', [visLampShgErr / max(onVisLampNum, 1) ], trainingLog , epoch, j )
        utils.writeErrToFile('visLampRender', [visLampRenErr / max(onVisLampNum, 1) ], trainingLog , epoch, j )
        ...
        if j - opt.iterId < 5000:
            # Write errors to screen
            visLampNumSum = max(np.sum(visLampNumNpList[1: j+1-opt.iterId ] ), 1 )

            utils.writeNpErrToScreen('visLampPointsAccu', np.sum(visLampPointsErrsNpList[1:j+1-opt.iterId, :], axis=0) / visLampNumSum, epoch, j )
            utils.writeNpErrToScreen('visLampShgAccu', np.sum(visLampShgErrsNpList[1:j+1-opt.iterId, :], axis=0) / visLampNumSum, epoch, j )
            utils.writeNpErrToScreen('visLampRenAccu', np.sum(visLampRenErrsNpList[1:j+1-opt.iterId, :], axis=0) / visLampNumSum, epoch, j )

            # Write errors to file
            utils.writeNpErrToFile('visLampPointsAccu', np.sum(visLampPointsErrsNpList[1:j+1-opt.iterId, :], axis=0) / visLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('visLampShgAccu', np.sum(visLampShgErrsNpList[1:j+1-opt.iterId, :], axis=0) / visLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('visLampRenAccu', np.sum(visLampRenErrsNpList[1:j+1-opt.iterId, :], axis=0) / visLampNumSum, trainingLog, epoch, j )
        else:
            visLampNumSum = max(np.sum(visLampNumNpList[j-4999-opt.iterId: j+1-opt.iterId ] ), 1 )

            utils.writeNpErrToScreen('visLampPointsAccu', np.sum(visLampPointsErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / visLampNumSum, epoch, j )
            utils.writeNpErrToScreen('visLampShgAccu', np.sum(visLampShgErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / visLampNumSum, epoch, j )
            utils.writeNpErrToScreen('visLampRenAccu', np.sum(visLampRenErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / visLampNumSum, epoch, j )

            # Write errors to file
            utils.writeNpErrToFile('visLampPointsAccu', np.sum(visLampPointsErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / visLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('visLampShgAccu', np.sum(visLampShgErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / visLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('visLampRenAccu', np.sum(visLampRenErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / visLampNumSum, trainingLog, epoch, j )
        ...
        if j == 1 or j% 2000 == 0:
            ...
            if onVisLampNum > 0:
                utils.writeLampBatch(
                        ...
                        visLampOns,
                        maxLampNum,
                        '{0}/{1}_visLampGt.ply'.format(opt.experiment, j) )
                utils.writeLampList(
                        visLampCenterPreds,
                        depthDS,
                        normalDS,
                ...
                vutils.save_image( (visLampShadingNoPreds ** (1.0/2.2) ).transpose(0, 1).reshape(batchSize * maxLampNum, 3, opt.envRow, opt.envCol ),
                    '{0}/{1}_visLampShadingNoPred.png'.format(opt.experiment, j), nrow=nrow )
        ...
        if j % 5000 == 0:
            # save the models
            torch.save( {'model': visLampNet.module.state_dict(), 'optim': opVisLamp.state_dict() },
                       '{0}/visLampNet_iter{1}.pth'.format(opt.experiment, j ) )

    trainingLog.close()

    # Save the error record
    np.save('{0}/visLampPointsError_{1}.npy'.format(opt.experiment, epoch), visLampPointsErrsNpList )
    np.save('{0}/visLampShgError_{1}.npy'.format(opt.experiment, epoch), visLampShgErrsNpList )
    np.save('{0}/visLampRenError_{1}.npy'.format(opt.experiment, epoch), visLampRenErrsNpList )
    np.save('{0}/visLampNum_{1}.npy'.format(opt.experiment, epoch), visLampNumNpList )
