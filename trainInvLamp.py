...
# Optimizer
opInvLamp = optim.Adam(invLampNet.parameters(), lr=1e-4, betas=(0.9, 0.999 ) )
if opt.isFineTune:
    opInvLamp.load_state_dict(invLampDict['optim'] )
...
for epoch in list(range(epochId, opt.nepoch ) ):
    ...
    for i, dataBatch in enumerate(brdfLoader ):
        ...
        invErr = (geoW * invLampPointsErr + sizeW * invLampSizeErr ) / max(onInvLampNum, 1) \
            + shgW * invLampShgErr \
            + renW * invLampRenErr
        ...
        utils.writeErrToScreen('invLampPoints', [invLampPointsErr / max(onInvLampNum, 1) ], epoch, j )
        utils.writeErrToScreen('invLampSize', [invLampSizeErr / max(onInvLampNum, 1) ], epoch, j )
        utils.writeErrToScreen('invLampShading', [invLampShgErr ], epoch, j )
        utils.writeErrToScreen('invLampRender', [invLampRenErr ], epoch, j )

        # Write errors to file
        utils.writeErrToFile('invLampPoints', [invLampPointsErr / max(onInvLampNum, 1) ], trainingLog, epoch, j )
        utils.writeErrToFile('invLampSize', [invLampSizeErr / max(onInvLampNum, 1) ], trainingLog, epoch, j )
        utils.writeErrToFile('invLampShading', [invLampShgErr ], trainingLog, epoch, j )
        utils.writeErrToFile('invLampRender', [invLampRenErr ], trainingLog, epoch, j )
        ...
        if j - opt.iterId < 5000:
            # Write errors to screen
            invLampNumSum = max(np.sum(invLampNumNpList[1 : j+1-opt.iterId ] ), 1 )

            utils.writeNpErrToScreen('invLampPointsAccu', np.sum(invLampPointsErrsNpList[1:j+1-opt.iterId, :], axis=0) / invLampNumSum, epoch, j )
            utils.writeNpErrToScreen('invLampSizeAccu', np.sum(invLampSizeErrsNpList[1:j+1-opt.iterId, :], axis=0) / invLampNumSum, epoch, j )
            utils.writeNpErrToScreen('invLampShgAccu', np.mean(invLampShgErrsNpList[1:j+1-opt.iterId, :], axis=0), epoch, j )
            utils.writeNpErrToScreen('invLampRenAccu', np.mean(invLampRenErrsNpList[1:j+1-opt.iterId, :], axis=0), epoch, j )

            # Write errors to file
            utils.writeNpErrToFile('invLampPointsAccu', np.sum(invLampPointsErrsNpList[1:j+1-opt.iterId, :], axis=0) / invLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('invLampSizeAccu', np.sum(invLampSizeErrsNpList[1:j+1-opt.iterId, :], axis=0) / invLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('invLampShgAccu', np.mean(invLampShgErrsNpList[1:j+1-opt.iterId, :], axis=0), trainingLog, epoch, j )
            utils.writeNpErrToFile('invLampRenAccu', np.mean(invLampRenErrsNpList[1:j+1-opt.iterId, :], axis=0), trainingLog, epoch, j )

        else:
            invLampNumSum = max(np.sum(invLampNumNpList[j-4999-opt.iterId : j+1-opt.iterId ] ), 1 )

            utils.writeNpErrToScreen('invLampPointsAccu', np.sum(invLampPointsErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / invLampNumSum, epoch, j )
            utils.writeNpErrToScreen('invLampSizeAccu', np.sum(invLampSizeErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / invLampNumSum, epoch, j )
            utils.writeNpErrToScreen('invLampShgAccu', np.mean(invLampShgErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0), epoch, j )
            utils.writeNpErrToScreen('invLampRenAccu', np.mean(invLampRenErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0), epoch, j )

            # Write errors to file
            utils.writeNpErrToFile('invLampPointsAccu', np.sum(invLampPointsErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / invLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('invLampSizeAccu', np.sum(invLampSizeErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0) / invLampNumSum, trainingLog, epoch, j )
            utils.writeNpErrToFile('invLampShgAccu', np.mean(invLampShgErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0), trainingLog, epoch, j )
            utils.writeNpErrToFile('invLampRenAccu', np.mean(invLampRenErrsNpList[j-4999-opt.iterId:j+1-opt.iterId, :], axis=0), trainingLog, epoch, j )
        ...
        if j == 1 or j% 2000 == 0:
            ...
            utils.writeLampBatch(
                    ...
                    maxLampNum,
                    '{0}/{1}_invLampGt.ply'.format(opt.experiment, j ) )
            vutils.save_image( (invLampShadingsNoBatch ** (1.0/2.2) ).transpose(0, 1).reshape(batchSize * maxLampNum, 3, opt.envRow, opt.envCol ),
                '{0}/{1}_invLampShadingNoGt.png'.format(opt.experiment, j), nrow=nrow )
            vutils.save_image( (invLampShadingsBatch ** (1.0/2.2) ).transpose(0, 1).reshape(batchSize * maxLampNum, 3, opt.envRow, opt.envCol ),
                '{0}/{1}_invLampShadingGt.png'.format(opt.experiment, j), nrow=nrow )

            vutils.save_image( (invLampShadingNoGt ** (1.0/2.2) ), '{0}/{1}_invLampShadingNoGtSelected.png'.format(opt.experiment, j), nrow=nrow )
            vutils.save_image( (invLampShadingNoPred ** (1.0/2.2) ), '{0}/{1}_invLampShadingNoPred.png'.format(opt.experiment, j), nrow=nrow )

            vutils.save_image( (invLampShadingGt ** (1.0/2.2) ), '{0}/{1}_invLampShadingGtSelected.png'.format(opt.experiment, j), nrow=nrow )
            vutils.save_image( (invLampShadingPred ** (1.0/2.2) ), '{0}/{1}_invLampShadingPred.png'.format(opt.experiment, j), nrow=nrow )

            vutils.save_image( (invLampRenderGt ** (1.0/2.2) ), '{0}/{1}_invLampRenderGtSelected.png'.format(opt.experiment, j), nrow=nrow )
            vutils.save_image( (invLampRenderPred ** (1.0/2.2) ), '{0}/{1}_invLampRenderPred.png'.format(opt.experiment, j), nrow=nrow )
            ...
            utils.writeLampBatch(
                    ...
                    np.ones( (batchSize, 1 ) ),
                    1,
                    '{0}/{1}_invLampPred.ply'.format(opt.experiment, j), )
        ...
        if j % 5000 == 0:
            # save the models
            torch.save( {'model': invLampNet.module.state_dict(), 'optim': opInvLamp.state_dict() },
                       '{0}/invLampNet_iter{1}.pth'.format(opt.experiment, j ) )

    trainingLog.close()

    # Save the error record
    np.save('{0}/invLampPointsError_{1}.npy'.format(opt.experiment, epoch), invLampPointsErrsNpList )
    np.save('{0}/invLampSizeError_{1}.npy'.format(opt.experiment, epoch), invLampSizeErrsNpList )
    np.save('{0}/invLampShgError_{1}.npy'.format(opt.experiment, epoch), invLampShgErrsNpList )
    np.save('{0}/invLampRenError_{1}.npy'.format(opt.experiment, epoch), invLampRenErrsNpList )
    np.save('{0}/invLampNum_{1}.npy'.format(opt.experiment, epoch), invLampNumNpList )
