...
opLightEncoder = optim.Adam(lightEncoder.parameters(), lr=1e-4, betas=(0.9, 0.999) )
opWeightDecoder = optim.Adam(weightDecoder.parameters(), lr=1e-4, betas=(0.9, 0.999) )
opAxisDecoder = optim.Adam(axisDecoder.parameters(), lr=1e-4, betas=(0.9, 0.999) )
opLambDecoder = optim.Adam(lambDecoder.parameters(), lr=1e-4, betas=(0.9, 0.999) )
if opt.isFineTune:
    opLightEncoder.load_state_dict(lightEncoderDict['optim'] )
    opWeightDecoder.load_state_dict(weightDecoderDict['optim'] )
    opAxisDecoder.load_state_dict(axisDecoderDict['optim'] )
    opLambDecoder.load_state_dict(lambDecoderDict['optim'] )
...
for epoch in list(range(epochId, opt.nepoch ) ):
    ...
    for i, dataBatch in enumerate(brdfLoader ):
        ...
        totalErr = lightW * lightErr + shgW * (shgErr + renErr )

        # Write errors to screen
        utils.writeErrToScreen('shg', [shgErr, renErr ], epoch, j )
        utils.writeErrToScreen('light', [lightErr ], epoch, j )

        utils.writeErrToFile('shg', [shgErr, renErr ], trainingLog, epoch, j )
        utils.writeErrToFile('light', [lightErr ], trainingLog, epoch, j )
        ...
        if j - opt.iterId < 1000:
            ...
            utils.writeNpErrToScreen('lightAccu', np.mean(lightErrsNpList[1:j+1 - opt.iterId, :], axis=0), epoch, j )

            # Write errors to file
            utils.writeNpErrToFile('shadingAccu', np.mean(shadingErrsNpList[1:j+1 - opt.iterId, :], axis=0), trainingLog, epoch, j )
            utils.writeNpErrToFile('lightAccu', np.mean(lightErrsNpList[1:j+1 - opt.iterId, :], axis=0), trainingLog, epoch, j )
        else:
            # Write errors to screen
            utils.writeNpErrToScreen('shadingAccu', np.mean(shadingErrsNpList[j-999-opt.iterId : j+1-opt.iterId, :], axis=0 ), epoch, j )
            utils.writeNpErrToScreen('lightAccu', np.mean(lightErrsNpList[j-999-opt.iterId:j+1-opt.iterId, :], axis=0 ), epoch, j )

            # Write errors to file
            utils.writeNpErrToFile('shadingAccu', np.mean(shadingErrsNpList[j-999-opt.iterId:j+1-opt.iterId, :], axis=0 ), trainingLog, epoch, j )
            utils.writeNpErrToFile('lightAccu', np.mean(lightErrsNpList[j-999-opt.iterId:j+1-opt.iterId, :], axis=0 ), trainingLog, epoch, j )
        ...
        if j == 1 or j% 2000 == 0:
            ...
            utils.writeEnvToFile(envBatch, nrow, '{0}/{1}_envmapGt.hdr'.format(opt.experiment, j) )

            vutils.save_image( ( (shadingPred  )**(1.0/2.2) ).data,
                '{0}/{1}_shadingPred.png'.format(opt.experiment, j), nrow=nrow )
            utils.writeEnvToFile(envPred, nrow, '{0}/{1}_envmapPred.hdr'.format(opt.experiment, j ) )

        if j % 5000 == 0:
            # save the models
            torch.save( {'model': lightEncoder.module.state_dict(), 'optim': opLightEncoder.state_dict() },
                       '{0}/lightEncoder_iter{1}.pth'.format(opt.experiment, j ) )
            torch.save( {'model': weightDecoder.module.state_dict(), 'optim': opWeightDecoder.state_dict() },
                       '{0}/weightDecoder_iter{1}.pth'.format(opt.experiment, j ) )
            torch.save( {'model': axisDecoder.module.state_dict(), 'optim': opAxisDecoder.state_dict() },
                       '{0}/axisDecoder_iter{1}.pth'.format(opt.experiment, j ) )
            torch.save( {'model': lambDecoder.module.state_dict(), 'optim': opLambDecoder.state_dict() },
                       '{0}/lambDecoder_iter{1}.pth'.format(opt.experiment, j ) )


    trainingLog.close()
    # Save the error record
    np.save('{0}/shadingError_{1}.npy'.format(opt.experiment, epoch), shadingErrsNpList )
    np.save('{0}/lightError_{1}.npy'.format(opt.experiment, epoch), lightErrsNpList )
